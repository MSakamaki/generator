 ...
 *
 * @property {Object}  env         - 実行時の環境変数
 * @property {Object}  args        - 初期化時の引数
 * @property {String}  resolved    - generatorのパス
 * @property {String}  generatorName
 * @property {String}  description - `--help`オプションで使用する文言。
 * @property {String}  appname     - アプリケーション名
 * @property {Storage} config      - [.yo-rc.json](http://yeoman.github.io/generator/Storage.html)ファイルマネージャ
 * @property {Object}  src         - file utilインスタンスのスコープを`sourceRoot`(ソースコード)に設定
 * @property {Object}  dest        - file utilインスタンスのスコープをdestinationRoot(リリース先)に設定
 * @property {Function} log       - [adapter](https://github.com/yeoman/generator/blob/master/lib/env/adapter.js)を経由し、ログの内容を標準出力する。
 *
 * @example
 ...
 * classに引数を追加し、それ用のattribute getterを作成します。
 * 
 * 引数は、オプションにより様々な物が設定されます。
 * 最初に行われるのは、コマンドラインからの応答により判断される場合です。
 * 
 * オプションは、すべてのハッシュ(this.opthions)に保存され、引数はプロパティー(this.argument)としてコードの内部で使用できるようになります。
 *
 * ### Options:
 *
 *   - `desc` 引数の説明
 *   - `required` 必須項目（Boolean)
 *   - `optional` オプション項目かどうか(Boolean)
 *   - `type` String, Number, Array, or Object等の型
 *   - `defaults` 引数の初期値
 *   - `banner` 使用上の注意(説明文
 *
 * @param {String} name
